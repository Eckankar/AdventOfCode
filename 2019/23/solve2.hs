...
runMachines fs im ln =
    if isNat && fmap tail ln == fmap tail ln'
    then seqHead tm
    else runMachines fs' im' ln'
    where is = map (seqHead . (im M.!)) addresses
          os = zipWith ($) fs is
          fs' = map (deRun . snd3) os
          ps = chunksOf 3 $ concatMap thd3 os

          isNat = null ps && M.null (M.filterWithKey (\k v -> k /= 255 && not (Seq.null v)) im'')
          im' = if isNat then M.insert 0 tm im'' else im''
          im'' = foldl queueUp (M.mapWithKey nextPacket im) ps
          ln' = if isNat then Just (seqHead tm) else ln
          ...
          nextPacket 255 v = v
          nextPacket _ v = seqTail v

          queueUp im [255,x,y] = M.insert 255 (Seq.singleton [x,y]) im
          queueUp im [a,x,y] = M.insertWith (flip (Seq.><)) a (Seq.singleton [x, y]) im
...
main = do
    ...
    print $ (!! 1) $ runMachines (replicate (length addresses) pf) im Nothing
