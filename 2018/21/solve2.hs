...
runProgram instMap ipReg regs
    ...
    | ip `elem` [28] && traceShow (ip, inst, regs) False = undefined
...
findN :: Int -> Int -> Int -> Int -> S.Set Int -> Int
findN origN n c lastN seen =
    if c >= 256
    then findN origN n' c' lastN seen
    else if S.member n' seen
    then lastN
    else findN origN origN (n' .|. 65536) n' (S.insert n' seen)
...
main = do
    ...
    print $ findN 4921097 4921097 65536 0 S.empty
