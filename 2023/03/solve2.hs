...
main = do
    ...
    let symbolNeighbors = map (S.toList . S.fromList . mapMaybe (flip M.lookup intPosMap) . neighborhood) $ M.keys symbolMap
    print $ sum $ map (product . map (intValueMap M.!)) $ filter ((== 2) . length) symbolNeighbors
