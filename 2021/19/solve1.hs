...
data Scanner = Scanner Int [Point]
    deriving (Eq, Show)
...
nnDist (Scanner n ps) = NN n $ S.fromList $ map findNN ps
...
isScanner n (Scanner m _) = n == m

tryAlign (Scanner x xps) (Scanner y yps) = foldl checkRotation Nothing allRotations
    where checkRotation (Just v) _  = Just v
          checkRotation Nothing  rs = Scanner y <$> listToMaybe ryps'
            where ryps = applyRotation yps rs
                  ryps' = [ ryps' | xs <- xps, ys <- ryps,
                                    let ryps' = map ((`pPlus` xs) . (`pMinus` ys)) ryps,
...
main = do
    ...
    print $ S.size $ S.fromList $ concatMap fromScanner $ alignScanners (S.singleton 0) overlaps' scanners' [sz]
