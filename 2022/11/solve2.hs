...
findMonkeyLcm is = foldl1 lcm $ map extractMod is
    where extractMod i = fromJust $ fmap read $ stripPrefix "Test: divisible by " $ dropWhile isSpace $ i !! 3


parseMonkey l is = Monkey n items mFun
    where [mline, itemline, opline, testline, trueline, falseline] =
          ...
          mFun x = (if x' `mod` d == 0 then t else f, x' `mod` l)
            where x' = opF (realize opM x) (realize opN x)
...
main = do
    ...
    let l = findMonkeyLcm input
    let monkies = map (parseMonkey l) input
    let monkeyCounts = M.fromList $ map (\m -> (monkeyN m, 0)) monkies
    let monkeyRuns = iterate runMonkies (monkies, monkeyCounts)
    print $ foldl (*) 1 $ take 2 $ reverse $ sort $ M.elems $ snd $ monkeyRuns !! 10000
