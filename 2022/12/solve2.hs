...
shortestPath :: WorldMap -> M.Map (Int, Int) Int -> MinHeap -> (Int, Int) -> Maybe Int
...
main = do
    ...
    let startIdxs = map fst $ filter ((== 'a') . snd) $ A.assocs worldMap
    let [(endIdx,_)]   = filter ((== 'E') . snd) $ A.assocs worldMap
    print $ minimum $ catMaybes $ map (\startIdx -> shortestPath worldMap M.empty (H.singleton (0, startIdx)) endIdx) startIdxs
