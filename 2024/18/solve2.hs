...
findConstrainingByte input mapSize knownGood knownBad
    | knownGood == knownBad-1 = knownBad-1
    | otherwise =
        if endIdx `M.member` dm
        then findConstrainingByte input mapSize mid knownBad
        else findConstrainingByte input mapSize knownGood mid
        where mid = (knownGood + knownBad) `div` 2
              m = makeMap mapSize $ take mid input
              endIdx = (mapSize, mapSize)
              dm = bfs m (M.fromList [((0,0), 1)]) [(0,0)] 1 
...
main = do
    ...
    let badEgg = findConstrainingByte input mapSize 1024 $ length input
    let badPoint = input !! badEgg

    showMap $ M.insert badPoint '%' $ makeMap mapSize $ take badEgg input
    print badPoint
