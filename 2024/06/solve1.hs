...
simGuard :: M.Map Point Char -> (Point, Point) -> M.Map Point Char
simGuard m (p, d) =
    if ps' == Nothing then m'
    else if ps' == Just '#' then simGuard m (p, rotate d)
    else simGuard m' (p', d)
...
main = do
    ...
    let m' = simGuard m guard
    showMap m'
    print $ M.size $ M.filter (== 'X') m'
