...
main = do
    ...
    let invalidUpdates = filter (not . validUpdate orderSet) updates
    let fixedUpdates = map (sortBy (sortOrdering orderSet)) invalidUpdates
    print $ sum $ map middleElement fixedUpdates
